...
private function CharTable($player)     //chartable with guid 
    { 
    $res = @mysql_query("SELECT * FROM `characters` WHERE guid='".mysql_real_escape_string($player)."' LIMIT 1;", $this->pvpdbconn); 
...
private function CharTable2($player)    //chartable with name 
    { 
    $res = @mysql_query("SELECT * FROM `characters` WHERE convert(name using utf8) COLLATE utf8_bin  = '".mysql_real_escape_string($player)."' LIMIT 1;", $this->pvpdbconn); 
    $chartable2 = @mysql_fetch_assoc($res); 
    return $chartable2; 
    } 
...
public function getPrimaryProfessions(){
	...
	if(!empty($this->data)){
		...
		$res = mysql_query("SELECT * FROM `character_skills` WHERE guid ='".$this->charid."';", $this->pvpdbconn); 
		while($arr = @mysql_fetch_assoc($res)){
...
public function getSearchTeamsResults($realmName)
    ...
    $res = mysql_query("SELECT a.name,a.type,a.BackgroundColor,a.EmblemStyle,a.EmblemColor,a.BorderStyle,a.BorderColor, a.rating, a.rank, a.seasonGames, a.seasonWins, a.weekGames, a.weekWins, c.race FROM `arena_team` a JOIN characters c ON c.guid = a.captainguid WHERE a.name LIKE '".$this->searchstring."%';", $this->pvpdbconn); 
    while($arr = mysql_fetch_assoc($res))
		{
...
public function getGuildIdplayer() 
    ...
    if(!empty($this->data)){
		...
		$res = mysql_query("SELECT guildid FROM guild_member WHERE guid = '".$this->charid."' LIMIT 1;", $this->pvpdbconn); 
...
public function getCharStats() 
    { 
		...
		return array( 
			...
			$res = mysql_query("SELECT * FROM `character_stats` WHERE guid = '".$this->charid."' LIMIT 1;", $this->pvpdbconn);
			$arr = mysql_fetch_assoc($res);
...
public function getArenaTeammember()
	...
	$res = mysql_query("
SELECT atm.guid, atm.weekGames, atm.weekWins, atm.seasonGames, atm.seasonWins, atm.personalRating, c.name, c.race, c.gender, c.class, g.name as gname FROM `arena_team_member` atm JOIN `characters` c on atm.guid=c.guid LEFT JOIN `guild_member` gm on gm.guid = c.guid LEFT JOIN `guild` g on g.guildid=gm.guildid WHERE atm.arenateamid='".$this->teamid."';", $this->pvpdbconn);
	while($arr = @mysql_fetch_assoc($res))
		{
