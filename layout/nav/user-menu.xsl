            ...
            <div class="user-mod" style="text-align: right;">
                ...
                <xsl:choose>                    
                    ...
                    <xsl:otherwise>
						...
						<em class="classId{$pinned/@classId} staticTip" onmouseover="setTipText('{$txtPinClass}')" style="margin: 2px 4px 0 0;"/>
        ...
        <xsl:otherwise>
        	<xsl:choose>
                <xsl:when test="not(/page/pageIndex)">
                	<a id="loginLinkRedirect" class="loginLink" href="{/page/@requestUrl}?login=1"><xsl:value-of select="$loc/strs/login/str[@id='armory.login.login']"/></a>                
                </xsl:when>
                <xsl:otherwise>
                    <div class="loginbg">
                        <a id="loginLinkRedirect" class="loginLink" href="{/page/@requestUrl}?login=1"><xsl:value-of select="$loc/strs/login/str[@id='armory.login.login']"/></a>
...
<xsl:template name="userMenu">	
   ...
   <xsl:variable name="bookmark" select="document('/vault/bookmarks.xml')/page/characters"/>
	...
	<div id="menuHolder">
        ...
        <div id="myCharacters">
        	<a id="changeLink" href="/vault/character-select.xml">[<xsl:value-of select="$loc/strs/common/str[@id='user.manage.settings']"/>]</a>
		  ...
		  <xsl:for-each select="$profile[@selected &lt; 3]">		 			  
				...
				<div class="menuItem charlist">
					 ...
					 <a href="/character-sheet.xml?{@url}" class="charName js-character-name">
					 	<em class="classId{@classId} staticTip" onmouseover="setTipText('{$txtClass}')"/>
		...
		<div id="myBookmarks">
		  	<!-- paging arrows --> 
				<div id="bmArrows">
					 <a id="bmBack" class="staticTip" onmouseover="setTipText('Previous');" href="javascript:void(0)"> </a>
					 <a id="bmFwd" class="staticTip" onmouseover="setTipText('Forward');" href="javascript:void(0)"> </a>										 
				</div>
