...
<xsl:template name="charactersTable"><style type="text/css" media="screen, projection">
	...
	<script type="text/javascript" src="/js/character/character-select.js"></script>
...
<xsl:template match="character" mode="printRow">
    ...
    <xsl:if test="not(@selected)">
        <tr>
            ...
            <td>
                <a href="/guild-info.xml?gn={@guild}&amp;r={@realm}"><xsl:value-of select="@guild" /></a>
            </td>
            <td style="text-align:left; padding-left:3px;">
                    ...
                    <div class="full-list notab">
                        <div class="info-pane">
                            <xsl:call-template name="charactersTable">
                                <xsl:with-param name="src" select="concat('/content/',$lang,'/ri-armory.xml')"/>
                            </xsl:call-template>
                        </div>
                    </div>
                </div>
