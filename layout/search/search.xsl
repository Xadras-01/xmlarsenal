...
<xsl:import href="/layout/item-filters.xsl" />
...
<xsl:template name="searchTabs">
	...
	<script type="text/javascript">
		$(document).ready(function(){	
			...
			//store filters
			<xsl:for-each select="searchResults/filters/filter">filterValues["<xsl:value-of select="@name" />"] = "<xsl:value-of select="@value" />";
			</xsl:for-each>			
			
			initSearchResults("<xsl:value-of select="tabs/@selected" />", "<xsl:value-of select="searchResults/@searchText" />", "<xsl:value-of select="searchResults/items/@pn" />", filterValues, "<xsl:value-of select="$relColIndex" />");
		});	
	</script>
	...
	<tbody>
		<xsl:for-each select="searchResults/characters/character">
			...
			<tr>
				...
				<td><a href="/guild-info.xml?r={@realm}&amp;gn={@guild}"><xsl:value-of select="@guild" /></a></td>
...
<xsl:template name="itemResults">
	...
	<tbody>
		<xsl:for-each select="searchResults/items/item">
			<xsl:sort select="@relevance" />
			<tr>
				<td><a id="{@id}" href="http://eu.wowarmory.com/item-info.xml?i={@id}" class="rarity{@rarity} staticTip itemToolTip" style="background: url('/wow-icons/_images/21x21/{@icon}.png') 0 0 no-repeat; padding: 2px 0 4px 25px;"><xsl:value-of select="@name" /></a></td>
				<td class="rightNum"><xsl:value-of select="filter[@name='itemLevel']/@value" /></td>
				<td>
					<xsl:choose>
						<xsl:when test="current()/filter[@name='source']/@areaName">
							<a href="search.xml?[source]=dungeon&amp;fl[dungeon]={current()/filter[@name='source']/@areaId}&amp;[boss]=all&amp;[difficulty]=all&amp;[type]=all&amp;searchType=items"><xsl:value-of select="current()/filter[@name='source']/@areaName" /></a>
						</xsl:when>
						<xsl:otherwise>
