...
<xsl:template match="flash">
    <xsl:call-template name="flash">
            ...
            <xsl:with-param name="flashvars" select="@flashvars"/>
            <xsl:with-param name="bgcolor" select="@bgcolor"/>
            <xsl:with-param name="menu" select="@menu"/>
            <xsl:with-param name="wmode" select="@wmode"/>
    </xsl:call-template>
</xsl:template>
...
<xsl:template name="flash">
    ...
    <xsl:param name="noflash"/>
		...
		if ((Browser.safari &amp;&amp; flashId=="flashback") || (Browser.linux &amp;&amp; flashId=="flashback")){//kill the searchbox flash for safari or linux
		   document.getElementById("searchFlash").innerHTML = '<div class="search-noflash"/>';
		}else
...
<xsl:template name="tabs">
	...
	<xsl:variable name="tabData" select="document('/layout/nav/tabs.xml')/tabGroups/tabGroup[@id=$tabGroup]"/>
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<div class="subTabs" style="height: 1px;">
				<div class="upperLeftCorner" style="height: 5px;"/>
				<div class="upperRightCorner" style="height: 5px;"/>			
			</div>	
		</xsl:otherwise>
	</xsl:choose>	
</xsl:template>
    ...
    <xsl:with-param name="param1">
        <xsl:choose>
            ...
            <xsl:choose>
                ...
                (<xsl:value-of select="@count"/>)<xsl:choose><xsl:when test="position() &lt; last() - 1">, </xsl:when><xsl:when test="position()=last()"/><xsl:otherwise> <xsl:text> </xsl:text> <xsl:value-of select="$loc/strs/achievements/str[@id='achv.and']"/><xsl:text> </xsl:text> </xsl:otherwise>
            </xsl:choose>
            </xsl:for-each>
...
<xsl:template name="related-info">
	...
	<xsl:for-each select="document($src)/relatedinfo">
		...
		<div class="module-block-right">
			<!-- <div id="ad_300x250" class="armory300"/>
			<xsl:if test="$lang != 'zh_cn' and $lang != 'zh_tw'">
						...
						<div class="module-lite">
							<ul>
								<xsl:for-each select="/relatedinfo/relatedlinks/relatedlink">
									<xsl:choose>
										<xsl:when test="@type = 'hr'"><div class="hr"/></xsl:when>
										<xsl:otherwise>
											<li>
												<xsl:attribute name="class">
													<xsl:choose>
												...
												<xsl:choose>
													...
													<xsl:when test="@type = 'hr'"><em/></xsl:when>
													<xsl:otherwise>
