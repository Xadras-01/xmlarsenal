...
<xsl:include href="/layout/tabs-template.xsl"/>
<xsl:include href="/layout/arena/header.xsl"/>
...
<!-- layout call -->
<xsl:template match="page/gameListingChart"><link rel="stylesheet" href="/css/arena-report.css" /><link rel="stylesheet" href="/css/datepicker.css" />
            ...
            <div class="parchment-content">
                <div class="list">
					<div class="tabs"> 
						...
						<xsl:variable name="tabData" select="document('/layout/tabs/arena-teams.xml')" />
						<xsl:variable name="teamUrlLink" select="/page/gameListingChart/arenaTeam/@teamUrl" />
						<xsl:for-each select="$tabData/tabs/tab">
						<div class="tab">
							<xsl:if test="@pageId = 'matchhistory'">
								<xsl:attribute name="class">selected-tab</xsl:attribute>
							</xsl:if>
							<a href="/{@link}?{$teamUrlLink}"><xsl:value-of select="$loc/strs/unsorted/str[@id=current()/@key]" /></a>
						</div>
						</xsl:for-each>
						<div class="clear"></div>
					</div>
...
<xsl:template name="gameGraph">
	...
	<script type="text/javascript" src="/js/arena-report/graph.js"></script>
    ...
    <div id="graphContainer">
        ...
        <div id="arenaFlashOuter">	
            <xsl:call-template name="flash">
                ...
                <xsl:with-param name="src" select="'/flash/graph.swf'"/>
					...
					<tr>
						...
						<td style="font-weight: normal;">
							...
							<a class="staticTip goToMatchReport" onmouseover="setTipText('{$loc/strs/arena/str[@id='goToMatchReport']}')" 
								href="/arena-game.xml?gid={@id}&amp;b={/page/gameListingChart/arenaTeam/@battleGroup}"> </a>
