...
<xsl:template name="head-content"><link rel="stylesheet" type="text/css" href="/css/battlegroups.css" />   
...
<xsl:template name="battlegroupsContent">
	...
	<xsl:variable name="teamSize">
		<xsl:choose>
			<xsl:when test="not(@ts)">2</xsl:when>
			<xsl:otherwise><xsl:value-of select="@ts" /></xsl:otherwise>
		</xsl:choose>
	</xsl:variable>
	...
	<div class="battlegroups">
		<div class="battlegroupList">
			...
			<div class="realmSelect">
				...
				<select onchange="arenaLadderLink(this.value, {$teamSize});">
					<option value="-1">--</option>
					<xsl:for-each select="battlegroup[not(@tournamentBattleGroup=1)]/realms/realm">
						<xsl:sort select="@name" />
						<option value="{../../@display}"><xsl:value-of select="@name" /></option>
					</xsl:for-each>
				</select>
			</div>
		...
		<div class="battlegroupList tournamentRealms">
			<div class="selectTitle" style="margin-bottom:15px;"><xsl:value-of select="$loc/strs/unsorted/str[@id='armory.arenatab.arenaTournament']" /></div>
			<div class="clear" />
			<xsl:for-each select="battlegroup[@tournamentBattleGroup='1']">
				<div class="battlegroup">
					<a href="/arena-ladder.xml?ts=3&amp;{@ladderUrl}" class="title tournamentRealm"><xsl:value-of select="@display" /></a>
				</div>
			</xsl:for-each>
			<div class="clear" />
		</div>
