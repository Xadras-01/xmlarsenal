		...
		<xsl:choose>
			<xsl:when test="$whichPage = 'team'">		
				<div style="float: left; margin: 10px 0 0 -25px">
					...
					<a class="smFrame staticTip" href="{$regionForumsURL}board.html?sid=1&amp;forumName={$docHeader/@realm}" onmouseover="setTipText('{$loc/strs/unsorted/str[@id='armory.forum.link.realm']}');">
					...
					<a class="smFrame staticTip" style="margin-left: -20px;" href="{$regionForumsURL}board.html?sid=1&amp;forumName={$docHeader/@battleGroup}" onmouseover="setTipText('{$loc/strs/unsorted/str[@id='armory.forum.link.battleGroup']}');">
						<!-- <xsl:if test="not($region = 'US' or $region = 'EU')"> -->	
							<xsl:attribute name="href">javascript:void(0)</xsl:attribute>
							<xsl:attribute name="class">smFrame</xsl:attribute>
							<xsl:attribute name="onmouseover"></xsl:attribute>						
