...
<xsl:import href="tabs-template.xsl"/>

<xsl:template name="head-content"><link href="/css/character/achievements.css" rel="stylesheet" type="text/css" /></xsl:template>
              ...
              <div class="f_nav_bgbtm">
               ...
               <xsl:variable name="realms" select="document('/vault/character-select.xml?sel=2')/page/characters/character/@realm"/>
                <xsl:if test="$realms">
                 ...
                 <div class="f_nav_stitle"><xsl:value-of select="$loc/strs/achievements/str[@id='serverfirsts.realmnav.otherregions']"/>:</div>
