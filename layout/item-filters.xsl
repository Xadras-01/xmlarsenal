...
<xsl:include href="item-subtype.xsl" />

<xsl:variable name="classesXml" select="document(concat('/data/', $lang, '/classes.xml'))" />
...
<xsl:variable name="factionsXml" select="document('/data/factions.xml')" />
<xsl:variable name="locFactions" select="document('/data/factionStrings.xml')" />

<xsl:variable name="advOptionsXml" select="document('/data/items/advoptions.xml')" />

<xsl:variable name="releasesXml" select="document('/data/releases.xml')" />
...
<xsl:template name="optionsPvPRewards">
  ...
  <div name="divErrorPvP" id="divErrorPvP">  	
	<div class="option-cont"><xsl:value-of select="$loc/strs/itemsOptions/str[@id='armory.item-search.pvpsource']"/></div>
      <div class="input-cont"><select id="faction" name="fl[pvp]">
		...
		<xsl:variable name="locpvpxml" select="document(concat('/data/items/pvp', $factionLetter, '.xml'))" />
    ...
    <div class="input-cont"><select id="dungeon" onChange="changedungeon(this.value)" onKeyUp="changedungeon(this.value)" name="fl[dungeon]">
      ...
      <xsl:for-each select="$releasesXml/releases/release">
		...
		<xsl:choose>
          ...
          <xsl:when test="$releaseId=2"><option id="dungeonemblemofheroism" name="dungeonemblemofheroism" value="emblemofheroism"><xsl:value-of select="$loc/strs/itemsOptions/str[@id='armory.item-search.select.eohr']"/></option><option id="dungeonemblemofvalor" name="dungeonemblemofvalor" value="emblemofvalor"><xsl:value-of select="$loc/strs/itemsOptions/str[@id='armory.item-search.select.eovr']"/></option><option id="dungeonemblemofconquest" name="dungeonemblemofconquest" value="emblemofconquest"><xsl:value-of select="$loc/strs/itemsOptions/str[@id='armory.item-search.select.eocr']"/></option></xsl:when>
		</xsl:choose>		
          <xsl:for-each select="$locDungeons/page/dungeons/dungeon[@release=$releaseId]">
            ...
            <option class="fake{$dungeonPath/@hasHeroic}" id="dungeon{$dungeonId}" name="dungeon{$dungeonId}" value="{$dungeonId}"><xsl:value-of select="$dungeonName" /> </option>
         </xsl:for-each>
	   </optgroup>
      </xsl:for-each>
	</select></div></div>
...
<xsl:template name="optionsBosses">
  ...
  <div class="filter-container" id="child{@id}">
 ...
 <div class="ao-cont" id="showHideAllComplexButton"><a class="ao-pop" href="javascript: toggleAllComplex();"><div id="replaceAllComplex"><xsl:value-of select="$loc/strs/itemsOptions/str[@id='armory.item-search.showadvanced']"/></div><em></em></a></div>  
  ...
  <div id="childUsableBy">
	...
	<select id="usbleBy" name="fl[usbleBy]" onChange="javascript: changeusbleBy(this.value)" onKeyUp="javascript: changeusbleBy(this.value)">
      ...
      <xsl:for-each select="$classesXml/classes/class">
        <xsl:variable name="classKey" select="@id" />	  
	    <option id="usbleBy{@id}" name="usbleBy{@id}" value="{@id}"><xsl:value-of select="$loc/strs/classes/str[@id=concat('armory.classes.class.', $classKey)]" /></option>
        </xsl:for-each>
	</select>
...
<script type="text/javascript" src="/js/items/functions.js"></script>
