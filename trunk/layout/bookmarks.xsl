...
<xsl:template name="singleBookmark">
    ...
    <div class="menuItem bmlist">
    	<a class="character-achievement staticTip" onMouseOver="setTipText('{$loc/strs/login/str[@id='armory.login.achievements']}');" href="/character-achievements.xml?{@url}"><xsl:value-of select="@achPoints"/></a>
    	
        <a href="javascript:void(0);" class="rmBookmark staticTip" onMouseOver="setTipText('{$loc/strs/login/str[@id='armory.login.bookmark.remove']}');"> </a>    
    
    	<em class="classId{@classId} staticTip" onmouseover="setTipText('{$txtClassbm}')"/>
